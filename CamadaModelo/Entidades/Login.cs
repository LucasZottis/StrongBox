...
namespace StrongBox.CamadaModelo.Entidades {
    public class Login {
        ...
        public Login(long codigo, string usuario, string observacao, byte tamanho, TipoDeSenha tipo, string senha, long codigoLocal) {
            ...
            if (string.IsNullOrEmpty(observacao)) {
                ...
                throw new LoginException("Campo observação deve ter de 4 à 30 caracteres.");
            } else this.Observacao = observacao; if (tamanho < 4 || tamanho > 20) throw new LoginException("Tamanho da deve ser entre 4 e 20 caracteres."); else this.Tamanho = tamanho;
            if (senha.Length != Tamanho) throw new LoginException("O tamanho da senha não pode ser diferente que o definido."); else this.Senha = senha;
